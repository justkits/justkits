...
Extends `HTMLAttributes<HTMLDivElement>`. (`id`, `role`, `aria-live` 제외)

- `id`, `role`, `aria-live`: 접근성 속성으로 컴포넌트 내부에서 자동 관리
...
- **data-state property**: `data-state="open"|"closed"`를 제공하여 CSS 트랜지션 및 애니메이션에 활용할 수 있도록 지원. 현재는 `@justkits/motion`을 사용할 것을 권장한다.
